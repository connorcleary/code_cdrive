...
def build_model():
    ...
    model_ws = os.path.join(os.getcwd(), "henry_layered_hori")
...
def extract_results(swt, hk):
    ucnobj = bf.UcnFile("henry_layered_hori\\MT3D001.UCN", model=swt)
    times = ucnobj.get_times()
    concentration = ucnobj.get_data(totim=times[-1])
    cbbobj = bf.CellBudgetFile("henry_layered_hori\\henry_layered_hori.cbc")
    ...
    headobj = bf.HeadFile("henry_layered_hori\\henry_layered_hori.hds")
    times = headobj.get_times()
    head = headobj.get_data(totim=times[-1])

    results_ws = os.path.join(os.getcwd(), "henry_layered_hori", "results")
