...
def cam_with_pumping_and_recovery(modelname, Lz, Lx, Ly, nlay, nrow, ncol, head, perlen, dt, nstp, qlay, qrow, qcol, q, pumpT, recT, recQmult):
    ...
    oc = flopy.modflow.ModflowOc(
        ...
        stress_period_data={(0, 0): ["save head", "save budget"], 
                            (0, nstp-1): ["save head", "save budget"],
                            (1, nstp-1): ["save head", "save budget",],
                            (2, nstp-1): ["save head", "save budget"]},
        compact=True
    )
    ...
    btn = flopy.mt3d.Mt3dBtn(
        swt,
        nprs=-1,
        # timprs=[perlen, perlen+pumpT, perlen+pumpT+recT],
        prsity=0.35, # can change this
        sconc= sconc, # can change this: to each area having different starti,
        chkmas=False,
        nprobs=10,
        nprmas=10,
        dt0=[perlen/15, pumpT/15, recT/15]
    )
